...
public class StudentDd extends SQLiteOpenHelper {
    ...
    /* These are the attributes for the main profile table. Each row will represent a student profile,
       and all of the Strings initialized below represent a column for the table name "profiles". */
    private static final String TABLE_NAME = "profiles";
    private static final String ID_COL = "id";
    private static final String COLOR_COL = "color";
    private static final String SHAPE_COL = "shape";

    // Each column is used to store each subject's progress bar.
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        // We execute all create-table statements.
        db.execSQL(mainTable);
        db.execSQL(drawingTable);
        db.execSQL(mathTable);
    }

    /* This method is called whenever the database version changes.
    ...
    /* updateProgress() calls getWritableDatabase() and uses the arguments passed
       (color, shape, subject, and progress) to ensure that we update the correct profile
       and the correct subject. Saves or updates progress for a chosen subject. */
    public void updateProgress(String color, String shape, String subject, int progress) {
        // We use getWritableDatabase() to allow inserting new data into the table.
        // We also create a ContentValues object to store the progress value for the correct profile and subject.
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();

        /* The switch case is very helpful for locating exactly which subject
           we are updating. Once we know which subject we are updating,
           then we use put() to insert the progress integer into the correct column. */
    ...
    /* getProgress() calls getReadableDatabase(). We use the switch-case to compare the empty
      string called col and assign it the correct progress column based on the subject.
      This method includes 3 parameters to know exactly which profile we are pulling progress from. */
    public int getProgress(String color, String shape, String subject){
        SQLiteDatabase db = this.getReadableDatabase();
        String col = "";

        /* We use the switch case to identify exactly which subject's progress
           we want to get. Then we set the empty string 'col' to the name
           of that progress column. */
        switch (subject.toLowerCase()) {
            case "alphabet":
        ...
        // We check if the column is null. If it is, then we return 0.
        if(col == null){
            db.close();
            return 0;
        }
        ...
        /* This checks if the cursor is valid and positioned on the first row.
       If it is, then we pull the integer inside that specific progress column
    ...
    /* saveBitmap() uses the arguments passed (color, shape, subject, and page)
       to ensure that we are saving bitmaps to the correct student profile and subject.
       This method is responsible for saving the individual bitmaps for the alphabet
       and counting activities as letter or number pages. */
    public void saveBitmap(String color, String shape, String subject, String page, Bitmap bitmap){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();

        /* We use put() to save the bitmap into the correct profile using the selected
           shape and color, and then narrow it down to the subject. */
        ...
        /* We check if the bitmap is not null. If it is not null, then we compress
           the bitmap into PNG format. The PNG is then converted into a byte array
           and stored in DRAWING_BITMAP_COL, which uses the BLOB data type. */
        if(bitmap != null){
        ...
        /* We use insertWithOnConflict to ensure that the composite key stays unique.
           If the drawing for that page already exists, then it will automatically
           be replaced. This prevents duplicate rows for the same page. */
        db.insertWithOnConflict(TABLE_DRAWINGS, null, values, SQLiteDatabase.CONFLICT_REPLACE);
        db.close();
    }

    /* loadBitmap() loads saved drawings by using the arguments passed such as
      color, shape, subject, and page. */
    public Bitmap loadBitmap(String color, String shape, String subject, String page){
        SQLiteDatabase db = this.getReadableDatabase();

        // We match all four key columns to ensure we load the correct bitmap.
        ...
        /* This if-statement checks if the cursor is not null and if it can move
           to the first available row. We then try to retrieve the PNG byte array,
